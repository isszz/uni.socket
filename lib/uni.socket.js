...
export default class Socket {
	...
	async emit(event, data = {}) {
		...
		if (this.SocketTask) {
			...
			let socketInitTimer = setInterval(() => {
				...
				} else if (!isAddBuffer[event]) {
					isAddBuffer[event] = true
					// 消息发送失败时将消息缓存
					_this.addBuffer(da)
				}
			}, 100)
	...
	async sendBufferRegister() {
		if (this._connectioned) {
			// 缓存池备份
			// const backup = JSON.parse(JSON.stringify(this._buffer_register))
			let backups = []
			const backup = this._buffer_register
			this._buffer_register.map((buffer, i) => {
				this.SocketTask.send({
					data: JSON.stringify(buffer),
					success: () => {
						backups = backup.slice(i, 1)
					},
				})
			})
...
var UniSocketError = (function (_super) {
