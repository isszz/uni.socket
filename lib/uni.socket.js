 ...
 * @LastEditTime: 2022-09-17 09:55:26
...
export default class Socket
{
	...
	off(event, handler, data) {
		...
		for(var property in handlers) {
			if(handlers.hasOwnProperty(property) && property == event) {
	...
	async emit(event, data) {
		...
		const send = (da) => {
			this.SocketTask.send(JSON.stringify(da))
		}
	...
	async sendBufferRegister() {
		if (this._connectioned) {
			for (var i = 0; i < this._buffer_register.length; i++) {
				var buffer = this._buffer_register[i] || null
				
				if(this.isObject(buffer)) {
					this.SocketTask.send(JSON.stringify(buffer))
					delete this._buffer_register[i]
				}
			}
		}
	}
